...
long long calcSumSegment(int a, int b){	      	//O(n)
  /// 0=< a,b =<10^6
  long long sum = 0;
  for(int i =a ; i<=b ; ++i) /// O(b-0) = O(b)
    sum += i;
  return sum;
}


long long calcSumSegment(int a, int b){	    /// O(n where n = max(a,b))  	//O(n)
  /// 0=< b =<10^6 0=<a=<10^9
  long long sum = 0;
...
void calArrayLog(){   /// nlog(n)
    for(int i=0;i<n;i++){ // O(n)
        calcLog(arr[i]); // O(log(n))
    }
}
